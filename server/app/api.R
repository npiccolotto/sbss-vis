...
get_region_guidance <- function(points) {
  ...
  if (num_points > 1) {
    K <- matrix(1, nrow=num_points, ncol=num_points)
    points.K <- SpatialBSS.snss::local_covariance_matrix(as.matrix(points), list(K), whitening = F)[[1]]
    diff_ev <- ev_diff(points.K)
...
get_kernel_guidance <- function(points, K) {
  ...
  points.K <-
    SpatialBSS.snss::local_covariance_matrix(as.matrix(points), list(K), whitening = F)[[1]]
  diff_ev <- ev_diff(points.K)
  return(list(
    type = 'kernel',
    diff_ev = diff_ev,
...
build_contiguity_matrix <- function() {
  ...
  contiguity_pairs <- lapply(1:nrow(pairs), function(r) {
    i <- pairs[r, 1]
    j <- pairs[r, 2]
...
make_cov_diff_regionalization <- function(contiguity_tree, num_regions = BLOCK_MAX_LEVEL) {
  ...
  repeat {
    ...
    edge_hgs <-
      lapply(splittable_edges, function(e) {
...
# Takes the contiguity tree and some split edges, makes a regionalization
util_split_edges_to_regionalization <- function(contiguity_tree, splitting_edges) {
  ...
  regionalization <- lapply(1:num_regions, function(i) {
    ...
    return(list(
      id = paste('cov_diff', num_regions, i, sep = '.'),
      index = i,
      depth = num_regions,
      patch = region.patch,
...
calc_variogs <- function(group='param', bins=20){
  if (group != 'param') {
    return(c())
  }
...
dataset.spatial <-
  sp::SpatialPointsDataFrame(dataset.original[, c('longitude', 'latitude')],
                             ...
                             proj4string = WGS84)
dataset.param.spatial <-
  sp::SpatialPointsDataFrame(dataset.original[,c('longitude', 'latitude')],
                             as.data.frame(dataset.param),
                             proj4string = WGS84)
spatial.voronoi <- SDraw::voronoi.polygons(dataset.spatial, range.expand = 0.025)
...
if (file.exists(guidancefile.path)) {
  ...
  tic("Variograms")
  dataset.param.variog <- calc_variogs(bins=40)
  toc()
  ...
  for  (col in colnames(dataset.param.spatial@data)) {
    spatial_summaries[['param']][[col]] <-
      get_image_pyramid(dataset.param.spatial[, col])
  }
  toc()
...
function() {
  return(dataset.param.variog)
}


util_matrix_to_str <- function(M) {
  return(apply(M, 1:2, as.character) %>% paste(., collapse = ','))
}

sbss_results <- hash::hash()

util_make_id_from_regions_and_kernels <- function(regions, kernels) {
  str_reg <- lapply(regions, function(region) {
    return(util_matrix_to_str(region))
  }) %>% paste(., collapse='/')
  str_ker <- util_matrix_to_str(kernels)
  return(digest::sha1(paste(str_reg,  str_ker, sep='.')))
}

#* @post /sbss
#* @serializer json list(na="null", digits=8)
function(req) {
  # this unnecessarily computes guidance for regions again, but it's cheap so don't matter
  # the good thing is that all returned regions have a selection mask to apply to the dataset
  usr_regionalization <-
    make_regions_from_usr(req$argsBody$regions)
  usr_kernels <- make_kernels_from_usr(req$argsBody$kernels)
  kernel_guidance <- calc_guidance(usr_regionalization, usr_kernels)
  id <- util_make_id_from_regions_and_kernels(req$argsBody$regions,req$argsBody$kernels)
  sbss_results[[id]] <- list(
    regions = usr_regionalization,
    kernels = kernel_guidance
  )
  return(id)
}
...
function(id) {
  return(list(
    regions=list(sbss_results[[id]]$regions),
    kernels=list(sbss_results[[id]]$kernels)
  ))
}
...
function(id, res) {
  ...
  sbss_param <- list(
    regions=sbss_results[[id]]$regions,
    kernels=sbss_results[[id]]$kernels
  )
  save(sbss_param, file=tf)
